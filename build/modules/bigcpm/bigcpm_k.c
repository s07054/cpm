...
{
  int s;
  u32 mapsz = vma->vm_end - vma->vm_start;
  if (!(vma->vm_flags & VM_SHARED)) {
    printk ("Mapping should be shared \n");
    return -EINVAL;
  }
  if ( ((vma->vm_pgoff << PAGE_SHIFT) + mapsz)  > bigcpm_info.size) {
    printk ("%s: Attempting to Map more than MAX pgoff=%x, mapsz=%x, size=%x\n",
      __FUNCTION__, (u32)vma->vm_pgoff, mapsz, bigcpm_info.size);
    return -EINVAL;
  }

  s = remap_pfn_range(vma, vma->vm_start,
